...
if (isset(Rotas::$pag[1])) {
    if (Rotas::$pag[1] > 0) {
        ...
        if ($turma->TotalDados() > 0) {
            ...
            $qtd_cap = (int) $turma->GetItens()[1]['Vaga_Alunos'];
            $vaga = $qtd_cap - $qtd_mat;
            $nota->GetNota();
//        var_dump($turma->GetItens());
            $nota_aluno->GetNotaAlunoID($id);
            ...
            $smarty->assign('NOTA_ALUNO', $nota_aluno->GetItens());
        } else {
...
//$smarty->assign('VOLTAR', Sistema::VoltarPagina());
        ...
        $smarty->assign('MODAL', "mODAL APAGAR " . $turma->GetItens()[1]['Codigo_Curso']);
