...
class Sistema {
    ...
    public static function Fdata_D_M_A($data) {
        // 2017-04-23 23/04/2017
        if (is_null($data)) {
            $data = "";
        } else {
    ...
    public static function Fdata_A_M_D($data) {
        // 2017-04-23 23/04/2017
        $data_correta = explode("-", $data);
        $data = $data_correta[2] . "/" . $data_correta[1] . "/" . $data_correta[0];
        return $data;
    ...
    static function CalculaIdade($nascimento) {
        if (empty($nascimento)) {
            $nascimento = Config::IDADE_ALUNO_BASE;
        }
    ...
    static function GerarPassword($qtyCaraceters = 8) {
    ...
    static function convertText($term) {
    ...
    static function combobox($arrDados, $valorSelecionado = null) {
        foreach ($arrDados as $value) {
            $selected = ($valorSelecionado == $key) ? "selected=\"selected\"" : null;
            echo '<option value="' . $arrDados['Codigo'] . '" selected=' . $selected . '> ' . $arrDados['Designacao'] . '</option>';
            echo "$value";
            echo "$arrDados";
        }
    }
